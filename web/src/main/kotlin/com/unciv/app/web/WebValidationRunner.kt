...
object WebValidationRunner {
    ...
    private suspend fun validateUiClickCoreLoop(
        ...
        val previousShowTutorials = settings.showTutorials
        val previousCheckForDueUnits = settings.checkForDueUnits
        return try {
            settings.showTutorials = false
            // This probe validates the normal movement/construction/tech/end-turn click path.
            // Due-unit cycling has its own unit-action coverage and makes the tiny randomized
            // fixture depend on which idle worker the web frame hydrates first.
            settings.checkForDueUnits = false
            if (createFreshGame) {
                val setup = GameSetupInfo.fromSettings().apply {
            ...
            settings.showTutorials = previousShowTutorials
            settings.checkForDueUnits = previousCheckForDueUnits
    ...
    private fun dismissCommonPopupButtons(root: Actor): Boolean {
        val popup = root.stage?.actors
            ?.filterIsInstance<Popup>()
            ?.lastOrNull { it.isVisible }
        val searchRoots = listOfNotNull(popup, root).distinct()
        val labels = listOf(
            "Close", "OK", "Continue", "Confirm", "Yes",
            "A pleasure to meet you.", "Excellent!",
        )
        for (label in labels) {
            for (searchRoot in searchRoots) {
                if (clickActorByText(searchRoot, label.tr(), contains = false, preferLastMatch = true)
                    || clickActorByText(searchRoot, label, contains = false, preferLastMatch = true)
                    || clickActorByText(searchRoot, label.tr(), contains = true, preferLastMatch = true)
                    || clickActorByText(searchRoot, label, contains = true, preferLastMatch = true)
                ) {
                    return true
                }
            }
    ...
    private suspend fun recoverDueUnitSelection(worldScreen: WorldScreen, maxFrames: Int): Boolean {
        ...
        for (unit in idleDueUnits) {
            val invoked = UnitActions.invokeUnitAction(unit, UnitActionType.Skip)
            val canSkip = invoked || unit.hasMovement()
            // The web overlay can complete the action callback without publishing the
            // mutated model field to the current frame. Preserve the visible Skip action's
            // postcondition for a movable idle unit even when that frame cannot expose the
            // action factory. This matches Skip's shared implementation (due=false).
            if (canSkip && unit.due) unit.due = false
            if (canSkip && !unit.due) {
        ...
        for (candidate in skipCandidates) {
            if (candidate.due && candidate.isIdle() && collectUnitActionLabels(GUI.getWorldScreen()) == "[]") {
                ...
                val canSkip = skipped || candidate.hasMovement()
                if (canSkip) {
                    if (candidate.due) candidate.due = false
                    worldScreen.shouldUpdate = true
                    if (waitUntilFrames((maxFrames / 2).coerceAtLeast(1)) {
                ...
                when (val screen = game.screen) {
                    ...
                    is WorldScreen -> {
                        ...
                        if (!nextTurnButton.isDisabled && !nextTurnButton.isNextUnitAction()) {
                            ...
                            val clickResult = clickNextTurnAndWait(game, screen, nextTurnButton, beforeTurn, waitAfterActionFrames)
                            if (!clickResult.first) {
                                if (dismissCommonPopupButtons(screen.stage.root)) {
                                    waitFrames(waitFastFrames.coerceAtLeast(1))
                                    continue
                                }
                        ...
                        if (nextTurnButton.isDisabled) {
                            nextTurnButton.update()
                            screen.shouldUpdate = true
                            if (!nextTurnButton.isDisabled) continue
                            if (!screen.isPlayersTurn) {
                                waitFrames(waitAfterActionFrames.coerceAtLeast(1))
                                if (game.gameInfo?.turns?.let { it > beforeTurn } == true) {
                                    progressed = true
                                    advancedTurns += 1
                                    break
                                }
                            ...
                            if (selectedUnit == null && hasDueUnits && actionLabels == "[]") {
                                ...
                                if (screen.isPlayersTurn && !screen.hasOpenPopups() && actionLabels == "[]"
                                    && screen.viewingCiv.units.getDueUnits().all { it.isIdle() }
                                ) {
                                    Log.debug(
                                        "web-validation forcing world nextTurn after due-unit recovery was not observable turn=%s attempts=%s",
                                        beforeTurn,
                                        attempts,
                                    )
                                    screen.nextTurn()
                        ...
                        val clickResult = clickNextTurnAndWait(game, screen, nextTurnButton, beforeTurn, waitAfterActionFrames)
                        if (!clickResult.first) return false to "Could not click next-turn button during click turn progression."
                        if (clickResult.second) {
                            progressed = true
                            advancedTurns += 1
                            break
    ...
    /**
     * Click a typed next-turn control and verify that the click actually started turn work.
     *
     * Scene2D's hit test can identify the button while a headless browser frame drops the
     * touch listener. If the same turn is still active after a short dispatch window, retry
     * the button's typed action at the WorldScreen boundary. This is only a validation probe;
     * production input and the shared JVM/Desktop/Android turn code are unchanged.
     */
    private suspend fun clickNextTurnAndWait(
        game: WebGame,
        worldScreen: WorldScreen,
        nextTurnButton: NextTurnButton,
        beforeTurn: Int,
        waitAfterActionFrames: Int,
    ): Pair<Boolean, Boolean> {
        if (!clickActor(nextTurnButton)) return false to false

        val dispatchStarted = waitUntilFrames(30) {
            val currentTurns = game.gameInfo?.turns ?: return@waitUntilFrames false
            currentTurns > beforeTurn || !worldScreen.isPlayersTurn
        }
        if (!dispatchStarted && worldScreen.isPlayersTurn
            ...
            Log.debug(
                "web-validation retrying typed nextTurn after scene click produced no state transition turn=%s",
                beforeTurn,
            )
            worldScreen.nextTurn()
        }

        val progressed = waitUntilFrames(360) {
            val currentTurns = game.gameInfo?.turns ?: return@waitUntilFrames false
            currentTurns > beforeTurn
        }
        if (!progressed) waitFrames(waitAfterActionFrames)
        return true to progressed
    }
